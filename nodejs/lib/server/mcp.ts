...
function loadConfig(): McpConfig | null {
  ...
  // Default to repo root (one above standalone CWD) AND CWD itself so the same
  // file works in dev (`next dev` from nodejs/) and standalone (CWD becomes
  // .next/standalone/ which then needs the file copied — but we also accept
  // the repo-root file directly via outputFileTracingRoot).
