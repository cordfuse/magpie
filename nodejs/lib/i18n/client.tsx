...
// every Magpie page resolves the same way. Adds `Secure` when on HTTPS
...
export function setLocaleAndReload(loc: string) {
  ...
  document.cookie = `magpie_locale=${encodeURIComponent(loc)};path=/;max-age=${oneYear};SameSite=Lax${secure}`
  // Verify the cookie actually stuck — surfaces silent rejections (some
  // browser policies refuse cookies in specific contexts).
  if (!document.cookie.split(';').some(c => c.trim().startsWith('magpie_locale='))) {
    console.warn('[magpie] locale cookie failed to set; the browser may have rejected it')
    return
  }
